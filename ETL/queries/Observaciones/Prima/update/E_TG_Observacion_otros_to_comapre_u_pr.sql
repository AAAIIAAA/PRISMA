...
[TITULO OBSERVACION] = B.[TÍTULO OBSERVACIÓN],
[OBSERVACION] = B.OBSERVACIÓN,
[RECOMENDACION] = B.RECOMENDACIÓN,
[PRIMERA RESPUESTA] = B.[PRIMERA RESPUESTA],
[UNIDAD RESPONSABLE] = B.[UNIDAD RESPONSABLE],
[PROPIETARIO] = B.PROPIETARIO,
[EMAIL PROPIETARIO] = B.[EMAIL PROPIETARIO],
[OBSERVADOR] = B.OBSERVADOR,
...
FROM PROYECTOSIAV2.dbo.TG_Observacion_to_compare C with (NOLOCK) 
INNER JOIN  PROYECTOSIAV2.dbo.[fn_TG_Observacion_otros_Obtener_Cambios]() U ON C.Identificador = U.Identificador
INNER JOIN 
(
SELECT 
	...
	dbo.[InitCap](ISNULL((SELECT U.FirstName + ' ' + U.LastName
    ...
    dbo.[InitCap](ISNULL(STUFF((SELECT '; ' + U.FirstName + ' ' + U.LastName 
...
ELSE CONVERT(DATE,FE.FechaEmision) END) [FECHA EMISIÓN],
	...
	LEFT JOIN (select ISNULL(PARSENAME(REPLACE(CODE,'-','.'),4)+ '-'+
       ...
       (CASE WHEN LEN(PARSENAME(replace(code,'-','.'),1)) = 2 THEN '20'+PARSENAME(replace(code,'-','.'),1) 
			ELSE PARSENAME(replace(code,'-','.'),1) END),CODE)CODE_AJUST,X.*
	...
	LEFT JOIN (SELECT S9.ProjectID IDPROJECT, 
			MAX(CASE O9.Flag WHEN 1 THEN DATEADD(HOUR,-5, O9.ActionDate) ELSE O9.ActionDate END) FechaEmision 
				...
				GROUP BY S9.ProjectID) AS FE
			ON FE.IDPROJECT  = P.ID
    
	WHERE R.StatusLID IN (2,3,13,9,10,11,12) AND R.TrackFlag = 1 
	AND (P.StatusLID = 8 OR P.Code  = 'PRE - 001') AND (P.CODE_AJUST LIKE '%ROP%' ) 
		AND (P.CODE_AJUST NOT LIKE '%Prueba%')
