...
INSERT INTO PROYECTOSIAV2.dbo.TG_all_Observacion 
SELECT 
'PS-' + CONVERT(VARCHAR,B.ID),
B.PROYECTO,
B.[NOMBRE PROYECTO],
B.[TÍTULO OBSERVACIÓN],
PROYECTOSIAV2.dbo.Clear_HTML(B.OBSERVACIÓN),
PROYECTOSIAV2.dbo.Clear_HTML(B.RECOMENDACIÓN),
PROYECTOSIAV2.dbo.Clear_HTML(B.[PRIMERA RESPUESTA]),
B.[UNIDAD RESPONSABLE],
PROYECTOSIAV2.dbo.[InitCap](B.PROPIETARIO),
B.[EMAIL PROPIETARIO],
PROYECTOSIAV2.dbo.[InitCap](B.OBSERVADOR),
PROYECTOSIAV2.dbo.[InitCap](B.COLABORADOR),
B.[FECHA EMISIÓN],
B.RIESGO,
B.[FECHA ESTIMADA],
B.[FECHA REVISADA],
...
FROM (SELECT Identificador FROM PROYECTOSIAV2.dbo.TG_ALL_OBSERVACION_TO_COMPARE WITH(NOLOCK)) A 
RIGHT JOIN (
	SELECT 
		R.ID [ID], 
		P.Code [PROYECTO],
