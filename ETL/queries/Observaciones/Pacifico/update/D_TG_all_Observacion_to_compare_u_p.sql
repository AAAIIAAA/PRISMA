...
FROM PROYECTOSIAV2.dbo.TG_ALL_Observacion_to_compare C with (NOLOCK) 
INNER JOIN  PROYECTOSIAV2.dbo.[fn_TG_ALL_Observacion_Obtener_Cambios]() U ON C.Identificador = U.Identificador
INNER JOIN 
(
SELECT 
		R.ID [ID], 
	...
	FROM TeamMateR12.dbo.TM_Recommendation R WITH (NOLOCK) 
		...
		LEFT JOIN (SELECT * FROM PROYECTOSIAV2.dbo.View_TG_Proyecto_FechaEmisión WHERE Compañia != 'PS') PFE ON REPLACE(PFE.[Código],' ','') = REPLACE(P.Code,' ','') COLLATE Modern_Spanish_CI_AS
		LEFT JOIN (
