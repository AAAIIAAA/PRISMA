...
def insertar_pri():
    ...
    ds1 = obtener_query_TIGA(os.path.join(QUERIES_PRIMA_RIESGOS_CONTROLES_PATH, 'Cambios_insertar_prima.sql'))
    ...
    for idx, rd in ds1.iterrows():
        ...
        if pd.isna(matriz.at[i, "Ponderado del Control"]) or matriz.at[i, "Ponderado del Control"] == '':
            matriz.at[i, "Ponderado del Control"] = 0 
    ...
    avance = 60
    print(f"Progreso: {avance}%")
    rows_marca_inicial_pri,flag = execute_query_TIGA(os.path.join(QUERIES_PRIMA_RIESGOS_CONTROLES_PATH, 'PRI_Marca_Inicial.sql'))
    print("Filas afectadas marca inicial: ",rows_marca_inicial_pri)
    
    avance = 80
    print(f"Progreso: {avance}%")
    rows_insert_tmp_to_compare_pri,flag = execute_query_TIGA(os.path.join(QUERIES_PRIMA_RIESGOS_CONTROLES_PATH, 'Insert_TG_TMP_to_compare_prima.sql'))
    print("Filas afectadas insert tg_to_compare: ",rows_insert_tmp_to_compare_pri)
    ...
    avance = 90
    print(f"Progreso: {avance}%")
    rows_delete_tmp_pri,flag = execute_query_TIGA(os.path.join(QUERIES_PRIMA_RIESGOS_CONTROLES_PATH, 'Cambios_delete_TG_TMP_prima.sql'))
    print("Filas afectadas delete tg_tmp: ",rows_delete_tmp_pri)

    avance = 100
    print(f"Progreso: {avance}%")
    rows_delete_tmp_to_compare_pri,flag = execute_query_TIGA(os.path.join(QUERIES_PRIMA_RIESGOS_CONTROLES_PATH, 'Delete_TMP_to_compare_prima.sql'))
    print("Filas afectadas delete tg_to_compare: ",rows_delete_tmp_to_compare_pri)
