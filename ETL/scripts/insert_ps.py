...
def insertar_ps():
    ...
    ds1 = obtener_query_TIGA(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, 'Cambios_insert.sql'))
    ...
    for idx, rd in ds1.iterrows():
        ...
        temp = open(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, f'PS_CCModificaciones.sql'), 'r', encoding='utf-8-sig').read()
        ...
        if not ds5.empty:
            ...
            matriz.at[i, "Impacto"] = cálculo_editar_ps(matriz.at[i, "Impacto (en US$ miles)"], "Impacto")
            modificado = 1
        else:
            matriz.at[i, "Impacto"] = riesgo_inherente_ps(rd["ID Proyecto"], rd["ID Riesgo"], rd["ID Control"], "Impacto")
            matriz.at[i, "Impacto (en US$ miles)"] = cálculo_riesgo_ps(matriz.at[i, "Impacto"], "Impacto", rd["IdPlan"])
        ...
        if not ds5.empty:
            ...
            matriz.at[i, "Frecuencia"] = cálculo_editar_ps(matriz.at[i, "Frecuencia (veces por año)"], "Frecuencia")
            modificado = 1
        else:
            matriz.at[i, "Frecuencia"] = riesgo_inherente_ps(rd["ID Proyecto"], rd["ID Riesgo"], rd["ID Control"], "Frecuencia")
            matriz.at[i, "Frecuencia (veces por año)"] = cálculo_riesgo_ps(matriz.at[i, "Frecuencia"], "Frecuencia", rd["IdPlan"])
        ...
        matriz.at[i, "IFC"] = ifc_ps(matriz.at[i, "IFC (en US$ miles)"], rd["IdPlan"])
        ...
        if str(matriz.at[i, "IFC (en US$ miles)"]) != "":
            matriz.at[i, "Control_Clave"] = cont_clave_ps(matriz.at[i, "IFC (en US$ miles)"], matriz.at[i, "Impacto (en US$ miles)"])
        ...
        if not pd.isnull(rd["Nombre de la Herramienta/Sistema"]) and rd["Nombre de la Herramienta/Sistema"]!='N/A':
            matriz.at[i, "Nombre de la Herramienta/Sistema"] = categoría_ps(rd["Nombre de la Herramienta/Sistema"])
        
        if not pd.isnull(rd["Unidad Responsable del Control"]) and rd["Unidad Responsable del Control"]!='N/A':
            matriz.at[i, "Unidad Responsable del Control"] = categoría_ps(rd["Unidad Responsable del Control"])

        if not pd.isnull(rd["I-P-V-R"]) and rd["I-P-V-R"]!='N/A':
            temp = categoría_ps(rd["I-P-V-R"])
        ...
        if not pd.isnull(rd["EX-IN-VA-PR-DO"]) and rd["EX-IN-VA-PR-DO"]!='N/A':
            temp = categoría_ps(rd["EX-IN-VA-PR-DO"])
        ...
        if not pd.isnull(rd["Tipo de Controles"]) and rd["Tipo de Controles"]!='N/A':
            matriz.at[i, "Tipo de Controles"] = categoría_ps(rd["Tipo de Controles"])

        if not pd.isnull(rd["Naturaleza del Control"]) and rd["Naturaleza del Control"]!='N/A':
            matriz.at[i, "Naturaleza del Control"] = categoría_ps(rd["Naturaleza del Control"])
        ...
        if not pd.isnull(rd["Ponderado del Control"]) and rd["Ponderado del Control"]!='N/A':
            matriz.at[i, "Ponderado del Control"] = categoría_ps(rd["Ponderado del Control"])
            matriz.at[i, "Ponderado del Control"] = matriz.at[i, "Ponderado del Control"][:2].strip()
            matriz.at[i, "Evaluación del Control"] = categoría_ps(rd["Ponderado del Control"])
        ...
        ds2 = observaciones_ps(rd["ID Procedimiento"])
        
        for k in range(6):
            if not ds2.empty:
                if not pd.isnull(ds2.iloc[0, k]) and ds2.iloc[0, k]!='N/A':
                    temp = categoría_ps(ds2.iloc[0, k])
                    temp = principios_c(temp)
                    if temp != 0:
        ...
        matriz.at[i, "IFC2"] = ifc_ps(matriz.at[i, "IFC (en US$ miles)2"], rd["IdPlan"])

        
        
        matriz.at[i, "FechaEmisión"] = rd["Emisión"].strftime("%d/%m/%Y")
        
        if matriz.at[i, "Referencia_del_Proceso"].startswith(("PS", "PPS", "EPV", "EPS")):
            matriz.at[i, "Negocio"] = "Negocio de Seguros"
        elif matriz.at[i, "Referencia_del_Proceso"].startswith("CRE"):
            matriz.at[i, "Negocio"] = "Crediseguro S.A."
        else:
        ...
        query2 = open(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, f'PS_CITipoRiesgo.sql'), 'r', encoding='utf-8-sig').read()
        ...
        tmp = open(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, f'PS_CC_Proceso.sql'), 'r', encoding='utf-8-sig').read()
        ...
        matriz.at[i, "Área Responsable"] = categoría_ps(rd["Área Responsable"])
    ...
    # Insertar datos en TG_TMP
    columnas = "INSERT INTO dbo.TG_TMP (" + ",".join([f"[{col}]" for col in matriz.columns]) + ") VALUES "
    ...
    avance = 60
    print(f"Progreso: {avance}%")
    rows_marca_inicial_ps = execute_query_TIGA(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, 'PS_CC_MarcaInicial.sql'))
    print("Filas afectadas en marca inicial: ",rows_marca_inicial_ps)

    avance = 80
    print(f"Progreso: {avance}%")
    rows_insert_to_compare_ps =  execute_query_TIGA(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, 'Insert_TMP_to_compare.sql'))
    print("Filas afectadas en insert tg_to_compare: ",rows_insert_to_compare_ps)

    avance = 90
    print(f"Progreso: {avance}%")
    rows_delete_tmp_ps =  execute_query_TIGA(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, 'Cambios_Delete_TG_TMP.sql'))
    print("Filas afectadas en delete tg_tmp: ",rows_delete_tmp_ps)

    avance = 100
    print(f"Progreso: {avance}%")
    rows_delete_tg_tmp_compare_ps =  execute_query_TIGA(os.path.join(QUERIES_PACIFICO_RIESGOS_CONTROLES_PATH, 'Delete_TMP_to_compare.sql'))
    print("Filas afectadas en delete tg_to_compare: ",rows_delete_tg_tmp_compare_ps)
